...
interface EnvVarScannerToolProps {
  repoUrl: string;
  envFileContent: string | null;
}

export function EnvVarScannerTool({ repoUrl, envFileContent }: EnvVarScannerToolProps) {
    ...
    const handleScan = async () => {
        ...
        try {
            const result = await scanEnvVarsAction({ repoUrl, envFileContent });
            if (!result) {
