...
export function FilePurposeClassifierTool({
  repoUrl,
  fileList,
}: {
  repoUrl: string
  ...
  const handleClassify = async () => {
    if (!selectedFile) {
      toast({
        ...
        description: 'Please choose a file from the list to classify.',
        variant: 'destructive',
      })
      return
    }
      ...
      {state.result && (
        <div className="p-4 bg-accent rounded-md border space-y-2">
          <h4 className="font-semibold flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-primary" /> AI Classification
          </h4>
