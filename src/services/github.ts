...
async function getRepoContent(owner: string, repo: string, path: string): Promise<any> {
  try {
...
export async function fetchRepoData(repoUrl: string) {
    ...
    // Fetch README, package.json, and root file list in parallel
    const [readmeContent, packageJsonContent, rootFiles, envExampleContent] = await Promise.all([
    ...
    const fileList = Array.isArray(rootFiles) ? rootFiles.map((file: any) => file.path) : [];
